...
public class UnremarkedPolicyQueryAction  extends ActionSupport{
	private static final long serialVersionUID = 1L;
	private static Logger logger = Logger.getLogger(UnremarkedPolicyQueryAction.class);
...
//		System.out.print(jsonMap);
		...
		session.put("unremarkedPolicies", currentPageRows);
		return SUCCESS;
	}
	...
	private List<FinancePolicy> doResults(Map<String, Object> session,Map<String,Object> jsonMap) throws ParseException{		
		...
		if(isNewQuery(session.get("policyQueryTimestamp"))){			
			...
			queryConditions.put("remarkScope", RemarkScope.UNREMARKED);
		...
		if(session.get("unremarkedPolicies") != null)
			currentPageRows = (List<FinancePolicy>)session.get("unremarkedPolicies");
		else
